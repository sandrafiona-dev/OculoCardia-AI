...
def calculate_fractal_dimension(Z):
    ...
    Z = Z[:n, :n]
    log_n = int(np.log2(n))
    if log_n < 2: return 1.42
    sizes = [2**i for i in range(log_n, 1, -1)]
    counts = []
    for size in sizes:
...
def analyze_retina(image_bytes: bytes):
    ...
    avr_val = max(0.0, min(100.0, float((0.85 - avr) * 200)))
    tortuosity_val = max(0.0, min(100.0, float((1.6 - fd) * 300)))
    ...
    if MODEL_LOADED:
        ...
        risk_score = (0.7 * float(model_risk_score)) + (0.3 * float(heuristic_risk))
    else:
        risk_score = float(heuristic_risk)
        
    # Custom rounding to avoid IDE overload errors
    risk_score = math.floor(float(max(0.0, min(100.0, risk_score))) * 10 + 0.5) / 10.0
    ...
    return {
        "metrics": {
            ...
            "avr": math.floor(float(avr) * 100 + 0.5) / 100.0,
            "fractalDimension": math.floor(float(fd) * 1000 + 0.5) / 1000.0,
...
def get_prevention_plan(risk_level, avr, fd):
    ...
    tips = personalized.get("tips")
    
    if isinstance(tips, list):
        if avr < 0.6:
            tips.append("Marker Alert: Low AVR suggests persistent arteriolar narrowing. Limit caffeine and high-stress environments.")
        
        if fd < 1.35:
            tips.append("Marker Alert: Reduced Fractal Complexity suggests thinning of the vessel tree. Increase Omega-3 intake.")
